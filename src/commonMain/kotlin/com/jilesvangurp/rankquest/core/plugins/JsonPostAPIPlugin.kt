...
class JsonPostAPIPluginFactory(
    ...
    override fun create(configuration: SearchPluginConfiguration): SearchPlugin {
        val settings = DEFAULT_JSON.decodeFromJsonElement(
            JsonPostAPIPluginConfig.serializer(),
...
class JsonPostAPIPlugin(
    ...
    override suspend fun fetch(searchContext: Map<String, String>, numberOfItemsToFetch: Int): Result<SearchResults> {
        val (response, duration) = measureTimedValue {  httpClient.post(searchUrl) {
            requestHeaders.forEach { (key, value) ->
                header(key, value)
            }
            accept(ContentType.Application.Json)
            setBody(TextContent(requestBodyTemplate.applySearchContext(searchContext).let {
                val o = DEFAULT_JSON.decodeFromString<JsonObject>(it)
                // get rid of newlines and other escaped crap
                DEFAULT_JSON.encodeToString(o)
                    ...
                    val searchResultList =
                        hits.map { hit ->
                            if (hit is JsonObject) {
                                ...
                                throw JsonPathError(jsonPathToHits)
                            }
                        }
